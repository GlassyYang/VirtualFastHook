...
static inline void InitJit() {
    ...
    jit_compile_method_ = (bool (*)(void *, void *, void *, bool)) enhanced_dlsym(jit_lib,
                                                                                  "jit_compile_method");
    ...
    void *compiler_options = (void *) ReadPointer(
            (unsigned char *) jit_compiler_handle_ + pointer_size_);
...
static inline void SetProfilingSaveEntryPoint(void *profiling, void *entry_point) {
    memcpy((unsigned char *) profiling + kProfilingSavedEntryPointOffset, &entry_point,
           pointer_size_);
}
...
static inline void *CreatTrampoline(int type) {
    ...
    trampoline = mmap(NULL, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANON | MAP_PRIVATE, -1,
                      0);
...
void DisableHiddenApiCheck(JNIEnv *env, jclass clazz) {
    if (kHiddenApiPolicyOffset > 0) {
        ...
        LOGI("Disable Hidden Api Check:%d",
             ReadInt32((unsigned char *) runtime_ + kHiddenApiPolicyOffset));
    }
}
...
jint Init(JNIEnv *env, jclass clazz, jint version) {
    ...
    new_local_ref_ = (jobject (*)(void *, void *)) enhanced_dlsym(art_lib,
...
void DisableJITInline(JNIEnv *env, jclass clazz) {
    ...
    void *art_compiler_options = (void *) ReadPointer(
            (unsigned char *) (*art_jit_compiler_handle_) + pointer_size_);

    memcpy((unsigned char *) art_compiler_options + 6 * pointer_size_, &max_units, pointer_size_);
    LOGI("DisableJITInline %d",
         ReadInt32((unsigned char *) art_compiler_options + 6 * pointer_size_));
}
...
bool IsCompiled(JNIEnv *env, jclass clazz, jobject method) {
    ...
    void *art_method = (void *) (*env)->FromReflectedMethod(env, method);
    void *method_entry = (void *) ReadPointer(
            (unsigned char *) art_method + kArtMethodQuickCodeOffset);
...
int CheckJitState(JNIEnv *env, jclass clazz, jobject target_method) {
    ...
    LOGI("TargetMethod:%p QuickToInterpreterBridge:%p hotness_count:%hd", art_method,
         art_quick_to_interpreter_bridge_, hotness_count);
...
jint DoFullRewriteHook(JNIEnv *env, jclass clazz, jobject target_method, jobject hook_method,
                       jobject forward_method, jobject head_record, jobject target_record,
                       jobject tail_record) {
    ...
    for (int i = 0; i < 3; i++) {
        LOGI("OriginalPrologue[%d] %x %x %x %x", i,
             ((unsigned char *) original_prologue)[i * 4 + 0],
             ((unsigned char *) original_prologue)[i * 4 + 1],
             ((unsigned char *) original_prologue)[i * 4 + 2],
             ((unsigned char *) original_prologue)[i * 4 + 3]);
    }

    memcpy(jump_trampoline + jump_trampoline_entry_index, &quick_hook_trampoline_entry,
           pointer_size_);
    for (int i = 0; i < jump_trampoline_len / 4; i++) {
        LOGI("JumpTrampoline[%d] %x %x %x %x", i, ((unsigned char *) jump_trampoline)[i * 4 + 0],
             ((unsigned char *) jump_trampoline)[i * 4 + 1],
             ((unsigned char *) jump_trampoline)[i * 4 + 2],
             ((unsigned char *) jump_trampoline)[i * 4 + 3]);
    }


    memcpy(quick_hook_trampoline + quick_hook_trampoline_target_index, &art_target_method,
           pointer_size_);
    memcpy(quick_hook_trampoline + quick_hook_trampoline_hook_index, &art_hook_method,
           pointer_size_);
    memcpy(quick_hook_trampoline + quick_hook_trampoline_hook_entry_index, &hook_entry,
           pointer_size_);
    memcpy(quick_hook_trampoline + quick_hook_trampoline_next_entry_index, &next_entry,
           pointer_size_);
    for (int i = 0; i < quick_hook_trampoline_len / 4; i++) {
        LOGI("QuickHookTrampoline[%d] %x %x %x %x", i,
             ((unsigned char *) quick_hook_trampoline)[i * 4 + 0],
             ((unsigned char *) quick_hook_trampoline)[i * 4 + 1],
             ((unsigned char *) quick_hook_trampoline)[i * 4 + 2],
             ((unsigned char *) quick_hook_trampoline)[i * 4 + 3]);
    }

    if (art_forward_method) {
        memcpy(quick_target_trampoline + quick_target_trampoline_original_index, original_prologue,
               original_prologue_len);
        memcpy(quick_target_trampoline + quick_target_trampoline_target_index, &art_target_method,
               pointer_size_);
        memcpy(quick_target_trampoline + quick_target_trampoline_original_next_entry,
               &original_next_entry, pointer_size_);
        for (int i = 0; i < quick_target_trampoline_len / 4; i++) {
            LOGI("QuickTargetTrampoline[%d] %x %x %x %x", i,
                 ((unsigned char *) quick_target_trampoline)[i * 4 + 0],
                 ((unsigned char *) quick_target_trampoline)[i * 4 + 1],
                 ((unsigned char *) quick_target_trampoline)[i * 4 + 2],
                 ((unsigned char *) quick_target_trampoline)[i * 4 + 3]);
        }
    }

    memcpy(quick_original_trampoline + quick_original_trampoline_target_index, original_prologue,
           original_prologue_len);
    memcpy(quick_original_trampoline + quick_original_trampoline_original_next_entry,
           &original_next_entry, pointer_size_);
    for (int i = 0; i < quick_original_trampoline_len / 4; i++) {
        LOGI("QuickOriginalTrampoline[%d] %x %x %x %x", i,
             ((unsigned char *) quick_original_trampoline)[i * 4 + 0],
             ((unsigned char *) quick_original_trampoline)[i * 4 + 1],
             ((unsigned char *) quick_original_trampoline)[i * 4 + 2],
             ((unsigned char *) quick_original_trampoline)[i * 4 + 3]);
    }

    if (art_forward_method) {
        memcpy((unsigned char *) art_forward_method + kArtMethodQuickCodeOffset, &forward_entry,
               pointer_size_);
        LOGI("Forward NewEntry:%p",
             ReadPointer((unsigned char *) art_forward_method + kArtMethodQuickCodeOffset));
    }

    __builtin___clear_cache(quick_hook_trampoline,
                            quick_hook_trampoline + quick_hook_trampoline_len);
    __builtin___clear_cache(quick_original_trampoline,
                            quick_original_trampoline + quick_original_trampoline_len);
    if (art_forward_method) {
        __builtin___clear_cache(quick_target_trampoline,
                                quick_target_trampoline + quick_target_trampoline_len);
    }
    ...
    if (current_handler_ == NULL) {
        ...
        memset(default_handler_, 0, sizeof(struct sigaction));
        memset(current_handler_, 0, sizeof(struct sigaction));
    ...
    long page_size = sysconf(_SC_PAGESIZE);
    unsigned alignment = (unsigned) ((unsigned long long) target_code % page_size);
    int ret = mprotect((void *) (target_code - alignment),
                       (size_t) (alignment + jump_trampoline_len),
                       PROT_READ | PROT_WRITE | PROT_EXEC);
    LOGI("Mprotect:%d Pagesize:%d Alignment:%d", ret, page_size, alignment);

    memcpy((unsigned char *) art_target_method + kArtMethodQuickCodeOffset,
           &art_quick_to_interpreter_bridge_, pointer_size_);
    memcpy(target_code, jump_trampoline, jump_trampoline_len);
    for (int i = 0; i < jump_trampoline_len / 4; i++) {
        LOGI("TargetCode[%d] %x %x %x %x", i, ((unsigned char *) target_code)[i * 4 + 0],
             ((unsigned char *) target_code)[i * 4 + 1], ((unsigned char *) target_code)[i * 4 + 2],
             ((unsigned char *) target_code)[i * 4 + 3]);
    }
    ...
    memcpy((unsigned char *) art_target_method + kArtMethodQuickCodeOffset, &target_entry,
           pointer_size_);

    __builtin___clear_cache(target_code, target_code + jump_trampoline_len);

    (*env)->SetLongField(env, head_record, kHookRecordClassInfo.jump_trampoline_,
                         (long) jump_trampoline);
    (*env)->SetLongField(env, target_record, kHookRecordClassInfo.quick_hook_trampoline_,
                         (long) quick_hook_trampoline);
    if (art_forward_method) {
        (*env)->SetLongField(env, target_record, kHookRecordClassInfo.quick_target_trampoline_,
                             (long) quick_target_trampoline);
    }
    (*env)->SetLongField(env, tail_record, kHookRecordClassInfo.quick_original_trampoline_,
                         (long) quick_original_trampoline);

    return 0;
}

jint DoPartRewriteHook(JNIEnv *env, jclass clazz, jobject target_method, jobject hook_method,
                       jobject forward_method, jlong quick_original_trampoline,
                       jlong prev_quick_hook_trampoline, jobject target_record) {
    void *art_target_method = (void *) (*env)->FromReflectedMethod(env, target_method);
    void *art_hook_method = (void *) (*env)->FromReflectedMethod(env, hook_method);
    void *art_forward_method = NULL;
    if (forward_method != NULL) {
    ...
    if (art_forward_method) {
        unsigned char original_prologue[original_prologue_len];
        memcpy(original_prologue, (unsigned char *) quick_original_trampoline +
                                  quick_original_trampoline_original_index, original_prologue_len);
        ...
        void *original_next_entry = (unsigned char *) quick_original_trampoline +
                                    quick_original_trampoline_next_entry_index;
        memcpy(quick_target_trampoline + quick_target_trampoline_original_index, original_prologue,
               original_prologue_len);
        memcpy(quick_target_trampoline + quick_target_trampoline_target_index, &art_target_method,
               pointer_size_);
        memcpy(quick_target_trampoline + quick_target_trampoline_next_entry_index,
               &original_next_entry, pointer_size_);
        for (int i = 0; i < quick_target_trampoline_len / 4; i++) {
            LOGI("QuickTargetTrampoline[%d] %x %x %x %x", i,
                 ((unsigned char *) quick_target_trampoline)[i * 4 + 0],
        ...
        void *forward_entry = CodePointToEntryPoint(quick_target_trampoline);
        memcpy((char *) art_forward_method + kArtMethodQuickCodeOffset, &forward_entry,
               pointer_size_);
        LOGI("Forward NewEntry:%p",
             ReadPointer((unsigned char *) art_forward_method + kArtMethodQuickCodeOffset));
    }
    ...
    memcpy((char *) art_target_method + kArtMethodQuickCodeOffset,
           &art_quick_to_interpreter_bridge_, pointer_size_);
    memcpy((void *) (prev_quick_hook_trampoline + quick_hook_trampoline_next_entry_index),
           &prev_next_entry, pointer_size_);
    for (int i = 0; i < quick_hook_trampoline_len / 4; i++) {
        LOGI("PrevQuickHookTrampoline[%d] %x %x %x %x", i,
             ((unsigned char *) prev_quick_hook_trampoline)[i * 4 + 0],
             ((unsigned char *) prev_quick_hook_trampoline)[i * 4 + 1],
             ((unsigned char *) prev_quick_hook_trampoline)[i * 4 + 2],
             ((unsigned char *) prev_quick_hook_trampoline)[i * 4 + 3]);
    }
    memcpy((char *) art_target_method + kArtMethodQuickCodeOffset, &target_entry, pointer_size_);

    __builtin___clear_cache((void *) prev_quick_hook_trampoline,
                            (unsigned char *) prev_quick_hook_trampoline +
                            quick_hook_trampoline_len);
...
jint DoReplaceHook(JNIEnv *env, jclass clazz, jobject target_method, jobject hook_method,
                   jobject forward_method, jboolean is_native, jobject target_record) {
    void *art_target_method = (void *) (*env)->FromReflectedMethod(env, target_method);
    void *art_hook_method = (void *) (*env)->FromReflectedMethod(env, hook_method);
    void *art_forward_method = NULL;
    if (forward_method != NULL) {
    ...
    memcpy((unsigned char *) hook_trampoline + hook_trampoline_target_index, &art_hook_method,
           pointer_size_);

    LOGI("HookTrampoline:%p HookMethod:%p TargetMethod:%p ForwardMethod:%p", hook_trampoline,
         art_hook_method, art_target_method, art_forward_method);
    for (int i = 0; i < hook_trampoline_len / 4; i++) {
        LOGI("HookTrampoline[%d] %x %x %x %x", i, ((unsigned char *) hook_trampoline)[i * 4 + 0],
             ((unsigned char *) hook_trampoline)[i * 4 + 1],
             ((unsigned char *) hook_trampoline)[i * 4 + 2],
             ((unsigned char *) hook_trampoline)[i * 4 + 3]);
    }

    if (art_forward_method) {
        memcpy((unsigned char *) target_trampoline + target_trampoline_target_index,
               &art_target_method, pointer_size_);
        memcpy((unsigned char *) target_trampoline + target_trampoline_target_entry_index,
               &target_entry, pointer_size_);

        if (kTLSSlotArtThreadSelf) {
            uint32_t hotness_count = GetArtMethodHotnessCount(art_target_method);
            LOGI("TargetTrampoline:%p TargetMethod:%p QuickToInterpreterBridge:%p",
                 target_trampoline, art_target_method, art_quick_to_interpreter_bridge_);
            if (hotness_count >= kHotMethodThreshold) {
        ...
        for (int i = 0; i < target_trampoline_len / 4; i++) {
            LOGI("TargetTrampoline[%d] %x %x %x %x", i,
                 ((unsigned char *) target_trampoline)[i * 4 + 0],
                 ((unsigned char *) target_trampoline)[i * 4 + 1],
                 ((unsigned char *) target_trampoline)[i * 4 + 2],
                 ((unsigned char *) target_trampoline)[i * 4 + 3]);
        }
    }

    memcpy((unsigned char *) art_target_method + kArtMethodQuickCodeOffset, &new_target_entry,
           pointer_size_);
    LOGI("Target NewEntry:%p",
         ReadPointer((unsigned char *) art_target_method + kArtMethodQuickCodeOffset));
    if (art_forward_method) {
        memcpy((unsigned char *) art_forward_method + kArtMethodQuickCodeOffset, &new_forward_entry,
               pointer_size_);
        LOGI("Forward NewEntry:%p",
             ReadPointer((unsigned char *) art_forward_method + kArtMethodQuickCodeOffset));
    }
    ...
    if (kArtMethodInterpreterEntryOffset > 0) {
        if (art_forward_method) {
            memcpy((unsigned char *) art_forward_method + kArtMethodInterpreterEntryOffset,
                   (unsigned char *) art_target_method + kArtMethodInterpreterEntryOffset,
                   pointer_size_);
        }
        memcpy((unsigned char *) art_target_method + kArtMethodInterpreterEntryOffset,
               (unsigned char *) art_hook_method + kArtMethodInterpreterEntryOffset, pointer_size_);
    }

    (*env)->SetLongField(env, target_record, kHookRecordClassInfo.hook_trampoline_,
                         (long) hook_trampoline);
    if (art_forward_method) {
        (*env)->SetLongField(env, target_record, kHookRecordClassInfo.target_trampoline_,
                             (long) target_trampoline);
    }
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    kHookRecordClassInfo.jump_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                               "mJumpTrampoline", "J");
    kHookRecordClassInfo.quick_hook_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                                     "mQuickHookTrampoline", "J");
    kHookRecordClassInfo.quick_original_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                                         "mQuickOriginalTrampoline",
                                                                         "J");
    kHookRecordClassInfo.quick_target_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                                       "mQuickTargetTrampoline",
                                                                       "J");
    kHookRecordClassInfo.hook_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                               "mHookTrampoline", "J");
    kHookRecordClassInfo.target_trampoline_ = (*env)->GetFieldID(env, hook_record_class,
                                                                 "mTargetTrampoline", "J");
